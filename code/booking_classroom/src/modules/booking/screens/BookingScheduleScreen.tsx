...
export function BookingScheduleScreen({ username, onBack }: { username: string; onBack: () => void }) {
  ...
  return <View style={styles.page}><ScreenHeader title="Lịch đặt phòng" onBack={onBack} /><View style={styles.tabs}>{(Object.keys(LABELS) as Filter[]).map(key => <Pressable key={key} onPress={() => setFilter(key)} style={[styles.tab, filter === key && styles.tabOn]}><Text style={[styles.tabText, filter === key && styles.tabTextOn]}>{LABELS[key]}</Text></Pressable>)}</View><ScrollView contentContainerStyle={styles.content}>{visible.length ? visible.map(item => <View style={styles.card} key={item.id}><BookingInfo booking={item} /></View>) : <Text style={styles.empty}>Không có lịch trong nhóm này.</Text>}<Text style={styles.note}>Màn hình này chỉ dùng xem lịch. Hủy, ủy quyền và quyền truy cập nằm trong “Quản lý đặt phòng”.</Text></ScrollView></View>;
}
const styles = StyleSheet.create({ page: { flex: 1, backgroundColor: '#F4F7FB' }, tabs: { backgroundColor: '#FFF', flexDirection: 'row', padding: 8 }, tab: { alignItems: 'center', borderRadius: 8, flex: 1, paddingHorizontal: 4, paddingVertical: 9 }, tabOn: { backgroundColor: '#B01432' }, tabText: { color: '#657084', fontSize: 11, fontWeight: '700', textAlign: 'center' }, tabTextOn: { color: '#FFF' }, content: { padding: 18, paddingBottom: 40 }, card: { backgroundColor: '#FFF', borderColor: '#E1E6EE', borderRadius: 12, borderWidth: 1, marginBottom: 11, padding: 14 }, empty: { color: '#657084', marginTop: 40, textAlign: 'center' }, note: { color: '#7A8494', fontSize: 12, lineHeight: 18, marginTop: 10, textAlign: 'center' } });
