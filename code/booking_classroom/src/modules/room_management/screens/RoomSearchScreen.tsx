...
import {
  BookingTimePicker,
  buildDateOptions,
} from '../../booking/components/BookingTimePicker';
import { getBookings } from '../../booking/services/bookingRepository';
import { saveBookingDraftSelection } from '../../booking/services/bookingDraftRepository';
...
export function RoomSearchScreen({ username, onBack }: Props) {
  ...
  const refreshMap = useCallback(async () => {
    if (!date) return;
    const candidates = (await getRooms())
  ...
  const markRoom = async () => {
    if (!selected || selected.mapStatus !== 'AVAILABLE') {
      setMessage('Chỉ có thể đánh dấu phòng đang trống trong khung giờ đã chọn.');
      return;
    }
    await saveBookingDraftSelection(username, {
      roomId: selected.id,
      date,
      startTime,
      endTime,
    });
    setMessage(`Đã đánh dấu ${selected.name}. Phòng và thời gian sẽ hiện sẵn ở chức năng Đặt phòng.`);
  };

  return (
    <View style={styles.page}>
      <ScreenHeader title="Tìm phòng theo sơ đồ tầng" onBack={onBack} />
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.mapSection}>
          <View style={styles.floorRail}>
            ...
            {[8, 7, 6, 5, 4, 3, 2, 1].map(item => (
              <Pressable key={item} onPress={() => setFloor(item)} style={[styles.floorButton, floor === item && styles.floorButtonOn]}>
            ...
            {MAP_ROWS.map((row, rowIndex) => (
              <View key={rowIndex} style={styles.mapRow}>
                {row.map((roomIndex, columnIndex) => {
                  ...
                  return <RoomCell key={room.id} room={room} selected={selectedId === room.id} onPress={() => setSelectedId(room.id)} />;
                })}
              </View>
            ))}
          </View>
        ...
        {selected ? (
          <View style={styles.selectedBox}>
            ...
            <Pressable disabled={selected.mapStatus !== 'AVAILABLE'} onPress={markRoom} style={[styles.markButton, selected.mapStatus !== 'AVAILABLE' && styles.disabled]}>
              <Text style={styles.markText}>Đánh dấu để đặt phòng</Text>
            </Pressable>
          </View>
        ) : null}
      </ScrollView>
...
function Input(props: React.ComponentProps<typeof TextInput> & { label: string }) {
  ...
  return <View><Text style={styles.inputLabel}>{label}</Text><TextInput {...rest} placeholderTextColor="#7D8795" style={styles.input} /></View>;
}
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: '#FFF', borderColor: '#CBD4E1', borderRadius: 8, borderWidth: 1, color: '#172033', paddingHorizontal: 9, paddingVertical: 9 },
  ...
  legend: { flexDirection: 'row', justifyContent: 'center', marginTop: 5 }, legendItem: { alignItems: 'center', flexDirection: 'row', marginHorizontal: 6 }, legendColor: { borderColor: '#BFC7D2', borderRadius: 3, borderWidth: 1, height: 13, marginRight: 4, width: 13 }, legendText: { color: '#596579', fontSize: 11 }, message: { color: '#315A86', fontSize: 12, marginVertical: 9, textAlign: 'center' },
  ...
  selectedBox: { backgroundColor: '#FFF', borderColor: '#D6DDE8', borderRadius: 11, borderWidth: 1, marginTop: 12, padding: 12 }, selectedTitle: { color: '#172033', fontSize: 16, fontWeight: '900' }, selectedDetail: { color: '#596579', fontSize: 12, marginTop: 4 }, markButton: { alignItems: 'center', backgroundColor: '#B01432', borderRadius: 8, marginTop: 10, paddingVertical: 11 }, markText: { color: '#FFF', fontWeight: '800' }, disabled: { opacity: 0.4 },
});
